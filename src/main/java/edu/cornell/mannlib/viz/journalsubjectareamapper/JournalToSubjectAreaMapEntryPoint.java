package edu.cornell.mannlib.viz.journalsubjectareamapper;
...
public class JournalToSubjectAreaMapEntryPoint {
	...
	private Map<String, String> forISSNMap = new HashMap<String, String>(); // <issn,
																			// subjectArea>
	private Map<String, String> forEISSNMap = new HashMap<String, String>();// <eissn,subjectArea>
	private Map<String, Set<String>> wosISSNMap = new HashMap<String, Set<String>>(); // <issn,
																						// list
																						// of
																						// subjectArea>
	private Map<String, Set<String>> wosEISSNMap = new HashMap<String, Set<String>>();// <eissn,list
																						// of
																						// subjectArea>
	private Map<String, String> subjectAreaMap = new HashMap<String, String>(); // <label,
																				// uri>
	private Map<String, Journal> schlrs_journals_map = new HashMap<String, Journal>(); // <uri
																						// of
																						// the
																						// journal,
																						// journal
	...
	public void runProcess() throws IOException, NoSuchAlgorithmException, InterruptedException {
		...
		LOGGER.info("JRNL2SA: merged_data size: "+ merged_data.size());
	...
	private void updateMasterFile(Set<String> newJournalURIs, File masterFile) throws IOException {
		...
		LOGGER.info("COLLAB1: updating the master journal id file...."+ masterFile.getCanonicalPath());
		int counter = 0;
		PrintWriter pw = null;
		FileWriter fw = new FileWriter(masterFile, true);
		pw = new PrintWriter(fw);
		for (String uri : newJournalURIs) {
			pw.print("\n\"" + uri + "\",\"" + Configuration.date + "\""); // id,
																			// source
			counter++;
		}
	...
	private Set<String> readMasterJournalFile(File file) throws IOException {
		LOGGER.info("COLLAB1: reading master journal file...."+ file.getCanonicalPath());
	...
	/**
	 * Main method that fills in the subject area data for any journal.
	 */
	private Set<Journal> fillSubjectAreaData() {
		...
		for (Journal schlrs_journal : journals) {
			...
			if (!issn.isEmpty() && forISSNMap.get(issn) != null) { // forISSNMap
																	// option1
				...
				filledFORCount++;
			} else if (!eissn.isEmpty() && forEISSNMap.get(eissn) != null) { // forEISSNMap
																				// option2
			...
			if (!issn.isEmpty() && wosISSNMap.get(issn) != null) { // wosISSNMap
																	// option3
				...
				filledWOSCount++;
			} else if (!eissn.isEmpty() && wosEISSNMap.get(eissn) != null) { // wosEISSNMap
																				// option4
	...
	private Set<String> readAndFilterSCHLJournalCSVFile(File file, Set<String> journalURI) throws IOException {
		LOGGER.info("JRNL2SA: Reading Scholars Journals data from "+file.getCanonicalPath());
		//LOGGER.info("JRNL2SA: journalURI set size: "+ journalURI.size());
		//if (journalURI.size() > 1 ) {
		//   Object[] objs = journalURI.toArray();
		//   LOGGER.info("JRNL2SA: journalURI first element: "+ objs[1]);
		//}
		...
		while ((line = br.readLine()) != null) {
			...
			while ((tokens = reader.readNext()) != null) {
				try {
					String uri = tokens[0];
					//LOGGER.info("scholars journal data uri: "+uri);
					String fixedUri = StringUtils.replace(uri, "<", "");
					fixedUri = StringUtils.replace(fixedUri, ">", "");
					//LOGGER.info("fixedUri: "+fixedUri);
					if (journalURI.contains(fixedUri) || fixedUri.equals("?journal")) continue;
					newIds.add(fixedUri);
					Journal new_obj = new Journal();
					new_obj.setUri(fixedUri);
		...
		br.close();
		LOGGER.info("JRNL2SA: lineCount " + lineCount);
	...
	private void readWOSJournalCSVFile(File file) throws IOException {
		LOGGER.info("JRNL2SA: Reading Web of Science Journals data from "+file.getCanonicalPath());
		BufferedReader br = null;
		String line = "";
		long lineCount = 0;
	...
	private void readFORJournalCSVFile(File file) throws IOException {
		LOGGER.info("JRNL2SA: Reading FOR Journals data from "+file.getCanonicalPath());
		BufferedReader br = null;
		String line = "";
		long lineCount = 0;
	...
	private void readSubjectAreasMap(File file) throws IOException {
		LOGGER.info("JRNL2SA: Reading subject area input file from "+file.getCanonicalPath());
		BufferedReader br = null;
		String line = "";
		long lineCount = 0;
		long subjCount = 0;
		...
		while ((line = br.readLine()) != null) {
			...
			while ((tokens = reader.readNext()) != null) {
				...
				try {
					//LOGGER.info("JRNL2SA: label: "+ tokens[0]);
					LOGGER.info("JRNL2SA: uri: "+ tokens[1]);
					String label = tokens[0];
					String uri = tokens[1];
					 
					if (!uri.startsWith("<"+ Configuration.VIVO_NAMESPACE)) 
						continue; // looking for only local URIs
	...
	private void saveJournalsRDF(Set<Journal> merged_data2, String filePath)
			throws InterruptedException, FileNotFoundException, NoSuchAlgorithmException {
